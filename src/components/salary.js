...
function calculateSalaryFrom(fullSalary) {
  ...
  const discountINSS = calculateDiscountINSS(baseINSS).toFixed(2);

  const baseIRPF = baseINSS - discountINSS;
  const discountIRPF = calculateDiscountIRPF(baseIRPF).toFixed(2);

  const netSalary = (baseINSS - discountINSS - discountIRPF).toFixed(2);

  const valuePercentageINSS = parseFloat(
    ((discountINSS / fullSalary) * 100).toFixed(2)
  );

  const valuePercentageIRPF = parseFloat(
    ((discountIRPF / fullSalary) * 100).toFixed(2)
  );

  const valuePercentageSalary = parseFloat(
    ((netSalary / fullSalary) * 100).toFixed(2)
  );
